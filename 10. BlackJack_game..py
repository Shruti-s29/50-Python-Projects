...
def choose_card():
    ''' Choose a card at random from a deck of cards '''
...
def play_game():
  ...
  #Deal the user and computer 2 cards each using choose_card()
  user_cards = []
  computer_cards = []
  is_game_over = False
  ...
  while not is_game_over:
    ...
    if user_score == 0 or computer_score == 0 or user_score > 21:
      ...
      # 10: If the game has not ended, ask the user if they want to draw another card. If yes, then use the choose_card() function to add another card to the user_cards List. If no, then the game has ended.
